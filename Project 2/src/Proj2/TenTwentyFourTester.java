...
public class TenTwentyFourTester {
	...
	public void resetShouldInitializeGameStatus() {
		gameLogic.reset();
		assertNotNull("reset() should initialize game status ",
				gameLogic.getStatus());
		assertEquals("reset() should initialize game status",
				GameStatus.IN_PROGRESS, gameLogic.getStatus());
	}
	...
	public void testSetValues() {
		...
		assertFalse("setValues() is not implemented correctly",
				zeroList.equals(oneList));
	}
	...
	public void resizeShouldHandleBoardsOfAnySize() {
		try {
			...
			for (int count = 0; count < REPEAT_COUNT; count++) {
				...
				assertEquals(
						"Your game logic should be able to handle game board of any size",
						rows * cols, gameLogic.getNonEmptyTiles().size());
			}
		} catch (Exception e) {
	...
	private void swiper(NumberSlider cm, SlideDirection dir, String chPat,
		...
		for (Cell c : out) {
			if (c.row < 0 || c.row >= after.length || c.column < 0
					|| c.column >= after[0].length)
				fail("game1024.Cell index (" + c.row + "," + c.column
						+ " is out of bound");
			assertTrue("getNonEmptyTiles() should return only non-zero cells",
					c.value != 0);
		...
		case 1:
			assertTrue(
					"move(): the new random value must show up in an empty cell",
					after[misMatchRow][misMatchCol] == 0);
			assertTrue("New generated value must be a power of 2",
					isPowerTwo(actual[misMatchRow][misMatchCol]));
			break;
		default:
			fail(action + " Mismatch entry at (" + misMatchRow + ","
					+ misMatchCol + ")\nExpected:\n" + afterSb
					+ "\nYour answer:\n" + actualSb);
	...
	private void noMoveSwiper(NumberSlider cm, SlideDirection dir, int[][] mat) {
		...
		if (dir != null) {
			...
			action = "move " + dir.name() + ":";
			assertFalse(
					"move() should return FALSE when no cells can be moved", b);
		}
		List<Cell> out = gameLogic.getNonEmptyTiles();
		int[][] actual = new int[mat.length][mat[0].length];
		for (Cell c : out) {
			if (c.row < 0 || c.row >= mat.length || c.column < 0
					|| c.column >= mat[0].length)
				fail("game1024.Cell index (" + c.row + "," + c.column
						+ " is out of bound");
			assertTrue("getNonEmptyTiles() should return only non-zero cells",
					c.value > 0);
			// assertTrue(action + " (" + c.row + "," + c.column +
			// ") should be zero",
	...
	public void slideShallReturnFalseWhenBoardUnchanged() {
		/*
		 * TODO: this test case does not work when getNonEmptyTiles returns a
		 * new ArrayList on each call
		 */
		...
		for (String pat : NOMOVE_PATTERNS) {
			...
			assertFalse(
					"move(UP) should return false when the board does not change",
			...
			assertFalse(
					"move(DOWN) should return false when the board does not change",
			...
			one = gameLogic.getNonEmptyTiles();
			assertFalse(
					"move(LEFT) should return false when the board does not change",
			...
			one = gameLogic.getNonEmptyTiles();
			assertFalse(
					"move(RIGHT) should return false when the board does not change",
	...
	public void testSwipeDown() {
		for (String[] pat : MOVE_PATTERNS) {
			swipeByPattern(SlideDirection.DOWN, reverse(pat[0]),
					reverse(pat[1]));
		}
	}
	...
	public void testSwipeRight() {
		for (String[] pat : MOVE_PATTERNS) {
			swipeByPattern(SlideDirection.RIGHT, reverse(pat[0]),
					reverse(pat[1]));
		}
	}
	...
	public void testIsGameBoardFullNoMoreMoves() throws Exception {
		...
		for (int count = 0; count < 10; count++) {
			...
			gameLogic.setValues(mat);
			assertEquals(
					"When no more moves are possible the game should be over",
					GameStatus.USER_LOST, gameLogic.getStatus());
		}
	...
	/*
	 * use DOT (.) for empty cells, be sure to limit the letters to a-f because
	 * the highest power of two use in the test is 32
	 */

	private final static String[] NOMOVE_PATTERNS = { ".......", "abcde",
			"a....", "ab...." };

	private final static String[][] MOVE_PATTERNS = {
	/* shift only, no merges */
	{ ".a.b.", "ab..." }, { "a....b", "ab...." }, { "a.bc...d", "abcd...." },
			{ "..a.b..a..b", "abab......." },

			/* merge and sum, use uppercase letter to indicate doubling */
			{ "abcdee", "abcdE." }, { "abbcdde", "aBcDe.." }, /* UNTESTED !!!!! */
			{ "a.b.bbc.ccd..", "aBbCcd......." }, { "abb..", "aB..." },
			{ "ab..b", "aB..." }, { "aa..bb..c", "ABc......" },
			{ "..a.a.aa..b", "AAb........" }, { "..a.b.cc..d", "abCd......." } };

	private final static String[] FULL_NOT_OVER = { "aabcde", "abbcde",
			"abccde", "abcdde", "abcdee" };
}
